...
export function httpStream(url: string, { body, proxy, jar, noRedirect, httpThrow=true, ...options }: XRequestOptions ={}) {
    ...
    return Object.assign(ret = new Promise<IncomingMessage>(async (resolve, reject) => {
        ...
        async function connect() {
            ...
            const newProm = pendingPromise<tls.TLSSocket>()
            if (pool.length >= 4)
                return seqRace = seqRace.then(async () => {
                    try { // race tracking the index
                        ...
                        pool[i] = newProm // replacing ensures the pool reflects the current usage of open sockets
            ...
            return new Promise<boolean>(resolve => {
                ;(proxyParsed.protocol === 'https:' ? https : http).request({
                    ...
                    if (res.statusCode !== 200)
                        return failed(res)
                    const tlsSocket = tls.connect({ socket, servername: parsed.hostname || undefined })
                    tlsSocket.on('close', disconnected)
                    tlsSocket.on('error', disconnected)
                    options.createConnection = () => tlsSocket
                    ret.catch(() => {}).then(() => {
                        newProm.resolve(tlsSocket)
                    })
                    resolve(true)
                }).on('response', failed).on('error', reject).end()

                function disconnected() {
                    _.pull(pool, newProm) // don't use "i" here, as it may change in time (other disconnections)
                }

                function failed(res: IncomingMessage) {
                    disconnected()
                    newProm.reject()
                    resolve(false)
                }
...
const proxySocketPools: Dict<Promise<tls.TLSSocket>[]> = {}
let seqRace = Promise.resolve()