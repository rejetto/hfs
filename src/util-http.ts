...
export function httpStream(url: string, { body, proxy, jar, noRedirect, httpThrow=true, ...options }: XRequestOptions ={}) {
    ...
    let ret: Promise<IncomingMessage>
    return Object.assign(ret = new Promise<IncomingMessage>(async (resolve, reject) => {
        ...
        async function connect() {
            ...
            async function handleNewProm(socket: SocketWithPoolRef) {
                options.createConnection = () => socket
                try {
                    const response = await ret // be sure the request is done but the response too, before reusing the same socket
                    await onFirstEvent(response, ['error', 'end', 'close'])
                }
                catch {}
