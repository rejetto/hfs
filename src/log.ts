...
import { DAY, makeNetMatcher, tryJson, Dict, Falsy, CFG, strinsert, repeat, formatTimestamp, HTTP_NOT_FOUND, HTTP_TEMPORARY_REDIRECT_KEEP_METHOD } from './misc'
import { urlToNode } from './vfs'
...
const logSpamRegex = defineConfig(CFG.log_spam_regex, /wlwmanifest.xml$|\.(php|asp)$|cgi|robots.txt$/.source, v => v ? new RegExp(v) : null)
...
export const logMw: Koa.Middleware = async (ctx, next) => {
    ...
    if (!logSpam.get()
        && (ctx.querystring.includes('{.exec|') // v2's bug
            ...
            || logSpamRegex.compiled()?.test(ctx.path)
                && (ctx.status === HTTP_NOT_FOUND
                    || ctx.status === HTTP_TEMPORARY_REDIRECT_KEEP_METHOD && !await urlToNode(ctx.path, ctx)))) {
