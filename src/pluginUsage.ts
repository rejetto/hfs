...
import { CFG, DAY, HOUR, repeat } from './misc'
...
const URL = process.env.HFS_PLUGIN_USAGE_URL || 'https://hfs-api.rejetto.com/plugin-usage'
...
async function send(method: 'POST' | 'DELETE', body: object) {
    await httpWithBody(URL + '/report', { method, body, timeout: 5000 })
}
