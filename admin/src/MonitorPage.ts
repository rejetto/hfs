...
function Connections() {
    ...
    const [showLog, showLogButton] = useToggleButton("Show log", "Hide log", v => ({
        icon: History,
    ...
    return h(Fragment, {},
        h(Box, { display: 'flex', alignItems: 'center', justifyContent: 'space-between' },
            ...
            useBreakpoint('md') && showLogButton,
        ),
        h(Grid, { container: true, flex: 1 },
            h(Grid, { item: true, sm: showLog ? 6 : 12 },
                h(DataTable, {
                    error,
                    rows,
            ...
            showLog && h(Grid, { item: true, xs: true }, h(Log))
        )
    )
}
