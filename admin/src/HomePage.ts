...
export default function HomePage() {
    ...
    const { list: plugins } = useApiList('get_plugins', {}, { reconnectGraceSeconds: 60 })
    const [checkPlugins, setCheckPlugins] = useState(false)
    const { list: pluginUpdates} = useApiList(checkPlugins && 'get_plugin_updates', {}, { reconnectGraceSeconds: 60 })
