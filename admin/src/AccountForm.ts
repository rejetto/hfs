...
export default function AccountForm({ account, done, groups, addToBar, reload }: FormProps) {
    ...
    return h(Form, {
        ...
        fields: [
            ...
            !isGroup && { k: 'require_password_change', comp: BoolField, xs: 12, sm: 6, lg: 6, helperText: "At first login" },

            { k: 'disable_password_change', label: "Password change", comp: SelectField, xs: 12, sm: 6, lg: isGroup ? 4 : 6,
                defaultValue: null,
                options: { [`Inherited (${values.canChangePassword ? 'allowed' : 'disabled'})`]: null, "Allowed": false, "Disabled": true },
            },
